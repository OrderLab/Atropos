...
// autocancel begin
#ifdef HAVE_AUTOCANCEL
#include "mysqld_autocancel.h"

THD *prev_killed_thd = nullptr;
struct timeval prev_killed_time;
#endif
// autocancel end
...
void add_global_thread(THD *thd)
{
  ...
  if (!have_thread)
  {
    ...
    //autocancel begin
    #ifdef HAVE_AUTOCANCEL
    /* it should be unnecessary to put here because we will check global thread list
    cancellable *c = createCancellable(0,thd->thread_id,0);
    */
    #endif
    //autocancel end
  }
...
{
#ifdef HAVE_AUTOCANCEL
  removeCancellable(threadMap, thd->thread_id);  //autocancel
#endif
...
#ifdef HAVE_AUTOCANCEL
      //cancellable *c = createCancellable(0,thd->thread_id,0);   // autocancel
#endif
...
// autocancel begin
#ifdef HAVE_AUTOCANCEL
// search for THS struct by its tid
THD *find_THD(unsigned long tid)
{
  Thread_iterator it= global_thread_list->begin();
  for (; it != global_thread_list->end(); ++it)
  {
...
void monitorProgress(unsigned long thread_id, FILE *fp)
{
  THD *thd = find_THD(thread_id);

  if (!thd || thd->thread_id == 0){
    return ;
  }

  fprintf(fp, "estimated rows: %lld\n", thd->estimatedRows);
}

void activate_cancel(unsigned long thd_id)
{
  char *should_kill_env;
  int should_kill = 0;
  should_kill_env = getenv("SHOULD_KILL");
  if (!should_kill_env){
		return ;
	}
  should_kill = atoi( should_kill_env );

  if (thd_id == 0){
    return ;
  }
  THD* cancel_thd = find_THD(thd_id);
  //THD* cancel_thd = detect_autocancellable_real();
  if (should_kill && cancel_thd){
    autocancel_kill(cancel_thd);
  }
  else if (should_kill && !cancel_thd){
    cancellable *c = (cancellable *) get(threadMap, (unsigned long) thd_id);
    if (!c){
      return ;
    }
    if (c->isBackground){
...
#endif
{
  ...
  */
  ...
  //pthread_create(&monitorTid, NULL, monitor, &pid);
	pthread_rwlock_init(&mapRWLock, 0);
	pthread_rwlock_init(&tidChangeMapRWLock, 0);
	pthread_rwlock_init(&thdLockMapRWLock, 0);
  pthread_key_create(&currTid, destructorFunction);
  pthread_create(&monitorSelfTid, NULL, initCancellableManagerNoSharedMemory, &pid);
  pthread_create(&monitorPerfTid, NULL, mysqlMonitor, &pid);
  //pthread_create(&monitorPerfTid, NULL, monitorPerfOutput, &pid);
  //pthread_create(&handleCrashTid, NULL, handleCrash, &pid);
  setMonitorProgress(monitorProgress);
  setCancelAction(activate_cancel);
  //setActivatePolicy(activate_policy);
#endif
  //autocancel end
...
{
  auto_cancel_flag = 1;
  FILE *fp = fopen("/tmp/rerun", "w");
  fprintf(fp, "%s\n", thd->query());
  fprintf(fp, "%d %s\n", thd->packet_length, thd->packet_command+1);
  fflush(fp);
  fclose(fp);
  strncpy(thd->rerun_packet_command, thd->packet_command, thd->packet_length);
  thd->rerun_packet_length = thd->packet_length;
  thd->rerun_packet_command[thd->rerun_packet_length] = '\0';
  thd->rerun_m_command = (enum enum_server_command) (uchar) thd->rerun_packet_command[0];       // we didn't not directly use m_command cause it's private in this context, this should be the way of set m_command
  thd->rerun_flag = 1;
  struct timespec tp;
  clock_gettime(CLOCK_REALTIME, &tp);
  thd->rerun_time = tp.tv_sec;
  kill_one_thread(thd, thd->thread_id, true);
}

// implementation of monitorProgress
/*
void monitorProgress(unsigned long thread_id, FILE *fp, cancellable *c)
{
  THD *thd = find_THD(thread_id);

  // special case, when find thd, it just end.
  if (!thd || thd->thread_id == 0){
    return ;
  }
  fprintf(fp, "%s\n", thd->query());
  ...
  if (lex){
    if (!thd->lex){
      fprintf(fp, "No row count!!!\n");
      c->finishedRows = 0;
      c->estimatedRows = 0;
      return;
    }
    SELECT_LEX *l = thd->lex->current_select;
    if (!l){
      fprintf(fp, "No row count!!!\n");
      c->finishedRows = 0;
      c->estimatedRows = 0;
    ...
    unsigned long rowcount = join->best_rowcount;
    unsigned long examined_rows = join->examined_rows;
    fprintf(fp, "examined row: %lu\nrow count: %lu\n", examined_rows, rowcount);
    c->finishedRows = examined_rows;
    c->estimatedRows = rowcount;
  }
  else{
    fprintf(fp, "No row count!!!\n");
    c->finishedRows = 0;
...
bool add_resource_usage(std::set<candidate> &candidate_set, long long appTid, cancellable *v, FILE *fp)
{
  /*
  struct candidate cd;
  int resources = getResources(v);
  for(int i = 0; i < resources; i++) {
    long long normalAvgUsage = getUsage(v,i);
    cd.appTid = appTid;
    cd.c = v;
    cd.benifit[i] = normalAvgUsage;
    candidate_set.insert(cd);
    return 1;
  }
  */
...
void new_generate_candidate_set(std::set<candidate> &candidate_set, FILE *fp)
{
  // pre-check if we need to consider lock wait
  hashMap* cancellables = getCancellables();

  for (unsigned int i = 0; i < cancellables->arraySize; ++i){
    if (cancellables->mapArray[i].inUse){
      long long appTid = cancellables->mapArray[i].key;
      cancellable *v = (cancellable *) cancellables->mapArray[i].value;
      if (!v->isCancellable){
        continue;
      }

      if (add_resource_usage(candidate_set, appTid, v, fp)){
        continue;
      }
    }
  }
}
long long new_pick_final_dominator(std::set<benefit> &dominator_set, FILE *fp)
{
  std::set<benefit>::iterator b;
  long long answer = -1;
  long long max_benefit = -1;
  for (b = dominator_set.begin(); b != dominator_set.end(); b++){
    long long current_benefit = b->I_cpu + b->I_app_mem;
...
THD *detect_autocancellable_real()
{
  THD *thd = NULL;
  FILE *fp = NULL;
  long long final_dominator;
  fp = fopen("/home/yigonghu/kill_detect_real", "a");
  fprintf(fp, "\nactivated\n");
  fflush(fp);

  // form candidate
  std::set<benefit> dominator_set;
  std::set<candidate> candidate_set;
  new_generate_candidate_set(candidate_set, fp);
  fflush(fp);
  fflush(fp);
  final_dominator = new_pick_final_dominator(dominator_set, fp);
  fprintf(fp, "final answer: %lld\n", final_dominator);
  fflush(fp);
  if (final_dominator == -1){
    return NULL;
  }

  thd = find_THD(final_dominator);
  fprintf(fp, "\n\n");
  fflush(fp);
  fclose(fp);
  return thd;
...
bool activate_policy(FILE *fp)
{
  struct candidate cd;
  long long AvgUsage = getThroughput();
  fprintf(fp, "normal throughput: %lld perf \n", AvgUsage);
  if (TPS_THRESHOLD >= AvgUsage){
    detect_autocancellable_real();
    return 1;
  }
...
/*
bool cpu_usage_candidate(std::set<candidate> &candidate_set, long long appTid, cancellable *v, FILE *fp)
{
  struct candidate cd;
  long long normalAvgUsage = v->normalAvgCpuTime;
  long long AvgUsage = averageHistoryCpuTime(v);
  fprintf(fp, "normal cpu usage: %lld perf warn cpu usage: %lld\n", normalAvgUsage, AvgUsage);
  if (normalAvgUsage <= AvgUsage){
    cd.appTid = appTid;
    cd.c = v;
    candidate_set.insert(cd);
    return 1;
...
bool app_mem_candidate(std::set<candidate> &candidate_set, long long appTid, cancellable *v, FILE *fp)
{
  fprintf(fp, "app mem queue: ");
  fprintQueue(&v->historyAppMemOccupyQueue, fp);
  struct candidate cd;
  long long size = v->historyAppMemOccupyQueue.size - 1;

  // check if there is a increasing trend in app mem usage
  long long prevAvgMemUsage = avgQueueInGap(&v->historyAppMemOccupyQueue, 0, size / 2);
  long long postAvgMemUsage = avgQueueInGap(&v->historyAppMemOccupyQueue, size / 2, size);

  if (postAvgMemUsage > prevAvgMemUsage){
    cd.appTid = appTid;
    cd.c = v;
    candidate_set.insert(cd);
    return 1;
...
void new_generate_candidate_set(std::set<candidate> &candidate_set, FILE *fp)
{
  ...
  for (unsigned int i = 0; i < threadMap->arraySize; ++i){
    if (threadMap->mapArray[i].inUse){
      long long appTid = threadMap->mapArray[i].key;
      cancellable *v = (cancellable *) threadMap->mapArray[i].value;
      if (!v->isCancellable){
        continue;
      }
      ...
      // cpu usage variation
      if (cpu_usage_candidate(candidate_set, appTid, v, fp)){
        continue;
      }

      // app mem variation
      if (app_mem_candidate(candidate_set, appTid, v, fp)){
        continue;
      }
    }
  }
...
void new_generate_benefit_set(std::set<benefit> &benefit_set, std::set<candidate> &candidate_set, FILE *fp)
{
  std::set<candidate>::iterator cd;
  int isCPUIdle = 0;
  ...
  fprintf(fp, "candidate set size: %lu\n", candidate_set.size());
  struct timeval t;
  gettimeofday(&t, NULL);
  fprintf(fp,"\ncurrent time: %lu\n", t.tv_sec*1000000 + t.tv_usec);
	unsigned long currentTime = t.tv_sec*1000000 + t.tv_usec;

  for (cd = candidate_set.begin(); cd != candidate_set.end(); ++cd){
    struct benefit b;
    double leftProgress;
    long long appTid = cd->appTid;
    int size;
    ...
    if (c->estimatedRows == 0 || c->finishedRows == 0){
      ...
      leftProgress = c->estimatedRows / c->finishedRows;
    }
    ...
    if (isCPUIdle){
      ...
      long long postAvgCpuUsage = averageQueue(&c->perfWarnCpuTimeQueue);
      b.I_cpu = postAvgCpuUsage * leftProgress + c->estimatedRows - c->finishedRows;
    }

    // app mem benefit
    size = c->historyAppMemOccupyQueue.size - 1;
    long long postAvgMemUsage = c->appMemOccupy;      // current task occupied mem
    b.I_app_mem = postAvgMemUsage * leftProgress;

    benefit_set.insert(b);
    fprintf(fp, "B tid: %lld\n", b.appTid);
    fprintf(fp, "B cpu: %f\n", b.I_cpu);
    fprintf(fp, "B app mem: %f\n", b.I_app_mem);
  }
}

/**
 * @brief when bl dom br, return true;
*/
/*
int is_dominate(const benefit &bl, const benefit &br)
{
  if (bl.I_cpu <= br.I_cpu && ((int) bl.I_cpu | (int) br.I_cpu)){
    return 0;
  }

  if (bl.I_app_mem <= br.I_app_mem && ((int) bl.I_app_mem | (int) br.I_app_mem)){
    return 0;
  }
...
// do multi-obj-opt on dominator set
void new_opt_benefit_set(std::set<benefit> &dominator_set, std::set<benefit> &benefit_set, FILE *fp)
{
  std::set<benefit>::iterator bl;
  std::set<benefit>::iterator br;
  for (bl = benefit_set.begin(); bl != benefit_set.end(); bl++){
    int should_insert = 1;
    // for every loop, check if bl dom br. if it is, remove br
    for (br = benefit_set.begin(); br != benefit_set.end(); br++){
      if (is_dominate((*br), (*bl))){
        should_insert = 0;
        break;
      }
    }
    ...
    if (should_insert){
      dominator_set.insert((*bl));
    }
  }

  fprintf(fp, "final dominator set:\n");
  for (bl = dominator_set.begin(); bl != dominator_set.end(); bl++){
    fprintf(fp, "B tid: %lld\n", bl->appTid);
    fprintf(fp, "B cpu: %f\n", bl->I_cpu);
    fprintf(fp, "B app mem: %f\n", bl->I_app_mem);
  }
...
void new_generate_dominator_set(std::set<benefit> &dominator_set, std::set<candidate> &candidate_set, FILE *fp)
{
  std::set<benefit> benefit_set;
  new_generate_benefit_set(benefit_set, candidate_set, fp);
  new_opt_benefit_set(dominator_set, benefit_set, fp);
}
...
THD *detect_autocancellable_real()
{
  ...
  FILE *fp = NULL;
  long long final_dominator;
  fp = fopen("/home/catbeat/kill_detect_real", "a");
  fprintf(fp, "\nactivated\n");
  fflush(fp);
  ...
  new_generate_candidate_set(candidate_set, fp);
  fflush(fp);
  new_generate_dominator_set(dominator_set, candidate_set, fp);
  fflush(fp);
  final_dominator = new_pick_final_dominator(dominator_set, fp);
  fprintf(fp, "final answer: %lld\n", final_dominator);
  fflush(fp);
...
void handle_connections_sockets()
{
  ...
  */
...
#ifdef HAVE_LIBWRAP
    {
      system("echo \"in LIBWRAP\n\" >> /home/yigonghu/test.txt");
  ...
  closeMonitor();
  //pthread_join(monitorTid, &res);
  pthread_join(monitorSelfTid, &res);
  pthread_join(monitorPerfTid, &res);
  pthread_key_delete(currTid);
  //pthread_join(handleCrashTid, &res);
#endif
  //autocancel end
  DBUG_VOID_RETURN;
}
