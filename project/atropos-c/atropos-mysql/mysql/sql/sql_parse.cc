...
extern "C"{
  #include <cancellable.h>      //autocancel
}
#include "mysqld_autocancel.h"  //autocancel
#include <unistd.h>             //autocancel
int auto_cancel_flag = 0;       //autocancel
#endif
...
bool do_command(THD *thd)
{
  ...
  /*
    indicator of uninitial
    ized lex => normal flow of errors handling
    (see my_message_sql)
  */
  ...
  */
  ...
  if (packet_length < 100){
    ...
    for (int i = 0; i < packet_length; ++i){
      thd->packet_command[i] = packet[i];
    }
    */
    strncpy(thd->packet_command, packet, packet_length);
    thd->packet_command[packet_length] = '\0';              //autocancel
    thd->packet_length = packet_length;                     //autocancel
  }
  clock_gettime(CLOCK_REALTIME, &tp);
  ctime = tp.tv_sec;
  if (thd->rerun_flag && ctime - thd->rerun_time >= 30){
    thd->rerun_flag = 0;
    return_value= dispatch_command(thd->rerun_m_command, thd, thd->rerun_packet_command+1, (uint) (thd->rerun_packet_length-1));
  }
#endif
  // autocancel end
		      ...
		      char* packet, uint packet_length)
{
  ...
  */
  ...
  //autocancel begin
#ifdef HAVE_AUTOCANCEL
  cancellable *c = getCancellable(thd->thread_id);
  taskStart(c);
  pid_t p = getpid();
  enableCancellable(p, c);
  //addRscOccupy((void *)c->tid, "mdlLockWait");
#endif
  //autocancel start
...
#ifdef HAVE_AUTOCANCEL  
  //getRscOccupy((void *)c->tid, "mdlLockWait");
  disableCancellable(c);    //autocancel
#endif
