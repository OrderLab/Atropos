	...
	ulint*		n_total_purged)	/*!< in/out: total pages purged */
{
	...
	do {
		...
		*n_total_purged += n_pages_purged;
		...
		// autocancel begin		test pause purge
		#ifdef HAVE_AUTOCANCEL
		if (c->needKilled){
			struct timespec tp;
			clock_gettime(CLOCK_REALTIME, &tp);
			//purge_killed_time = tp.tv_sec * 1000000000L + tp.tv_nsec;
			//trx_purge_stop();
			sleep(30);
		}
		#endif
						...
						required by os_thread_create */
{
	...
	pid_t p = getppid();
	c = createCancellable(0,tid,0);
	c->tid = tid;
	c->isBackground = 1;
  	insert(tidChangeMap, tid, (void *) tid);
	taskStart(c);
#endif
	//autocancel end
	...
	do {
		...
		if (srv_shutdown_state == SRV_SHUTDOWN_NONE
			...
			// autocancel begin
			/*
			if (purge_killed_time){
				struct timespec tp;
    			clock_gettime(CLOCK_REALTIME, &tp);
    			long long currentTime  = tp.tv_sec * 1000000000L + tp.tv_nsec;
				long long ti = currentTime - purge_killed_time;
				if (ti > 30000000000){
					trx_purge_run();
					purge_killed_time = 0;
				}
