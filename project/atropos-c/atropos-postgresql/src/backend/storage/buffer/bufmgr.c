			...
			bool *foundPtr)
{
	...
	if (buf_id >= 0)
	{
		...
		*foundPtr = true;
		...
		if (needTrackAnyway){
			addShmhtRscUsage(m, ntime - ptime, "buffer");
		}
		#endif
		// autocancel end
		return buf;
	...
	#ifdef HAVE_AUTOCANCEL
	wptime = tp.tv_sec * 1000000000L + tp.tv_nsec;		//autocancel
	#endif
	...
	for (;;)
	{
		...
		if (buf_id >= 0)
		{
			...
			*foundPtr = true;
			...
			if (needTrackAnyway){
				addShmhtRscUsage(m, ntime - ptime, "buffer");
				addShmhtRscUsageWait(m, ntime - wptime, "buffer");
			}
			#endif
			// autocancel end
			return buf;
