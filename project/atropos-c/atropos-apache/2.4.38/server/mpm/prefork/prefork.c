...
#include "stat.h"                   // autocancel
#include <setjmp.h>                 // autocancel

static sigjmp_buf autocancel_kill_back;         // autocancel
//extern char rerun_query_string[];		        // autocancel used for rerun
//extern char current_query_string[]; 	        // atuocancel used for rerun
//extern long long killed_time;			        // autocancel used for rerun
//extern long long current_query_string_length;	// autocancel
#endif
...
// autocancel begin
#ifdef HAVE_AUTOCANCEL
static void autocancel_keep_process_alive(int sig)
{
    fprintf(stderr, "long jmp\n");
    struct timespec tp;
    clock_gettime(CLOCK_REALTIME, &tp);
    killedTime = tp.tv_sec;
    strncpy(rerunQueryString, currQueryString, currQueryStringLength);
    rerunQueryString[currQueryStringLength] = '\0';
    rerunQueryStringLength = currQueryStringLength;
    FILE *fp = fopen("/tmp/debug", "a");
    fprintf(fp, "tid: %d\n", currPid);
    fprintf(fp, "curr length: %d\n", currQueryStringLength);
    fprintf(fp, "current: %s\n", currQueryString);
    fprintf(fp, "rerun: %s\n", rerunQueryString);
    fclose(fp);
    siglongjmp(autocancel_kill_back,1);
}
#endif
// autocancel end
    ...
    got_fd:
        ...
        if (current_conn) {
          ...
          uint64_t start_us, end_us;
            ...
            // autocancel begin
            #ifdef HAVE_AUTOCANCEL
            int res = sigsetjmp(autocancel_kill_back,1);
            if (res){
                fprintf(stderr, "jump back\n");
            }
            shmhtEndTask(m);		    // autocancel
            #endif
            // autocancel end
...
static int make_child(server_rec *s, int slot, int bucket)
{
    ...
    if (!pid) {
        ...
        #ifdef HAVE_AUTOCANCEL
        apr_signal(SIGUSR2, autocancel_keep_process_alive);     //autocancel
        #endif
        ...
        // autocancel begin
        #ifdef HAVE_AUTOCANCEL
        m = createCancellableTaskShmht(20, 10);	
		int pid = (int) getpid();			
		addShmhtTask(m, pid, 0);	
        #endif
        // autocancel end
...
// autocancel begin
#ifdef HAVE_AUTOCANCEL
void activate_cancel(unsigned long task_id){
    ...
    struct timespec tp;
    clock_gettime(CLOCK_REALTIME, &tp);
    killed_time = tp.tv_sec;
    strncpy(rerun_query_string, current_query_string, current_query_string_length);
    rerun_query_string[current_query_string_length] = '\0';
    FILE *fp = fopen("/tmp/debug", "a");
    fprintf(fp, "curr length: %d\n", current_query_string_length);
    fprintf(fp, "current: %s\n", current_query_string);
    fprintf(fp, "rerun: %s\n", rerun_query_string);
    fclose(fp);
    */

    kill(task_id, SIGUSR2);
    return ;
}
#endif
// autocancel end
...
static int prefork_run(apr_pool_t *_pconf, apr_pool_t *plog, server_rec *s)
{
    ...
    m = createCancellableTaskShmht(20, 10);	
	initCancellableTaskShmht(10, m);	
	printf("%s\n", "autocancel init");
	setClientMonitorMode();
	pthread_create(&monitorSelfTid, NULL, initCancellableManager, NULL); 
	pthread_create(&monitorPerfTid, NULL, pgsqlMonitor, NULL);
