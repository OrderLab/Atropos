...
mysql_execute_select(THD *thd, SELECT_LEX *select_lex, bool free_join)
{
  ...
  if (join->select_options & SELECT_DESCRIBE)
  {
    ...
    if (join->tables){
      // later we need to iterate each tab when it's multi-table
      JOIN_TAB *tab = join->join_tab;
      if (tab->position){
        TABLE *table = tab->table;
        SQL_SELECT *select= (tab->filesort && tab->filesort->select) ?
           tab->filesort->select : tab->select;
        if (select && select->quick){
          thd->estimatedRows = select->quick->records;
          setEstimatedJob(thd->thread_id, thd->estimatedRows);
        }
        else if (tab->type == JT_INDEX_SCAN || tab->type == JT_ALL){
          if (tab->limit){
            thd->estimatedRows= tab->limit;
            setEstimatedJob(thd->thread_id, thd->estimatedRows);
          }
          else
          {
            table->pos_in_table_list->fetch_number_of_rows();
            thd->estimatedRows= table->file->stats.records;
            setEstimatedJob(thd->thread_id, thd->estimatedRows);
          }
        }
      }
