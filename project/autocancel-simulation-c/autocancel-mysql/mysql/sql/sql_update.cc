                 ...
                 ha_rows *found_return, ha_rows *updated_return)
{
  ...
  */
  ...
  if (select && select->quick){
    thd->estimatedRows = rows2double(select->quick->records);
  }
  else if (!select && !using_filesort && limit != HA_POS_ERROR){
    thd->estimatedRows = rows2double(limit);
  }
  else{
    table->pos_in_table_list->fetch_number_of_rows();
    thd->estimatedRows= rows2double(table->file->stats.records);
  }
  setEstimatedJob(thd->thread_id, thd->estimatedRows);
#endif
  // autocancel end
  ...
  */
  ...
  while (!(error=info.read_record(&info)) && !thd->killed)
  {
    thd->inc_examined_row_count(1);
#ifdef HAVE_AUTOCANCEL
    addFinishedJob(thd->thread_id, 1);    //autocancel
#endif
    bool skip_record;
