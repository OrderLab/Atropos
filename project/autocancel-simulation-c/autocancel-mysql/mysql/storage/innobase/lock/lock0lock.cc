	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	if (wait_for != NULL) {
		...
		ib_lock_t *l = lock_table_create(table, mode | flags, trx);			//autocancel: get the return value
		// autocancel: TODO: add lock occupy api here
		#ifdef HAVE_AUTOCANCEL
		addRscOccupy(l, "tableLockWait");
		#endif
...
#ifdef HAVE_AUTOCANCEL
			getRscOccupy(lock, "tableLockWait");				// autocancel: TODO: release table lock here so getOccupy here
#endif
