...
static void report_intermediate(void)
{
  ...
  */
  if (ck_pr_load_double(&sb_globals.report_interval) == 0)      //autocancel project: change from uint to double
    return;
...
void print_run_mode(sb_test_t *test)
{
  ...
  {
    log_text(LOG_NOTICE, "Report intermediate results every %f second(s)",        //autocancel project: %d to %f
...
static void *report_thread_proc(void *arg)
{
  ...
  #define DOUBLE_SEC2NS(sec) (uint64_t) ((double) (sec) * NS_PER_SEC)
  const unsigned long long interval_ns = DOUBLE_SEC2NS(sb_globals.report_interval);
...
static int run_test(sb_test_t *test)
{
  ...
  ck_pr_store_double(&sb_globals.report_interval, 0);     // autocancel project: change from uint to double
