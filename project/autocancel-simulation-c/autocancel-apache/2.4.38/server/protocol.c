...
char default_ab_str[] = "ab -s 60 -t 110 -c 1 -n 1 -k http://127.0.0.1:8080";   // autocancel
char default_postfix[] = " &";                                                    // autocancel
int default_ab_str_length = 50;                             // autocancel
#endif
// autocancel begin
#ifdef HAVE_AUTOCANCEL
                                          ...
                                          int flags, apr_bucket_brigade *bb)
{
    ...
    do {
             ...
             e = APR_BUCKET_NEXT(e))
        {
            ...
            #ifdef HAVE_AUTOCANCEL
            if (e->start == 0){
                currQueryStringLength = len;
                if (currQueryStringLength < 512){
                    memcpy(currQueryString, str, currQueryStringLength);
                    currQueryString[currQueryStringLength] = '\0';
                }
                else{
                    currQueryStringLength = 0;
                }
            }
            #endif
            // autocancel end
            ...
            if (do_alloc) {
                ...
                if (!*s) {
                    ...
                    if (killedTime != 0){
                        ...
                        ctime = tp.tv_sec;
                        if (ctime - killedTime > 30){
                            char final_rerun_str[512];
                            char *result = NULL;
                            strncpy(final_rerun_str, default_ab_str, default_ab_str_length);
                            fprintf(stderr, "ready to rerun\n");
                            fprintf(stderr, "rerun: %s\n", rerunQueryString);
                            strncpy(final_rerun_str+default_ab_str_length, rerunQueryString+4, rerunQueryStringLength-15);
                            strncpy(final_rerun_str+default_ab_str_length+rerunQueryStringLength-15, default_postfix, 2);
                            final_rerun_str[default_ab_str_length + rerunQueryStringLength-13] = '\0';
                            result = replace(final_rerun_str, "?", "\\?");
