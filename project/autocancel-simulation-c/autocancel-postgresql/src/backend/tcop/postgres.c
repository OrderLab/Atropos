...
char rerun_query_string[512];	// autocancel used for rerun
char current_query_string[512]; // atuocancel used for rerun
long long current_query_string_length = 0;	// autocancel
long long killed_time = 0;		// autocancel used for rerun
#endif
				...
				ParamListInfo boundParams)
{
	...
	foreach(query_list, querytrees)
	{
		...
		// autocancel begin
#ifdef HAVE_AUTOCANCEL
		if (stmt->planTree && stmt->planTree->plan_rows != 0)
			shmhtSetEstimatedJob(stmt->planTree->plan_rows);
#endif
		// autocancel end
...
#ifdef HAVE_AUTOCANCEL
	//shmhtStartTask(m);			//autocancel
	//shmhtSetEstimatedJob(portal->cplan->stmt_list)
#endif
	// autocancel end
...
quickdie(SIGNAL_ARGS)
{
	...
	PG_SETMASK(&BlockSig);
#ifdef HAVE_AUTOCANCEL
	removeShmhtTask(m, getpid());	//autocancel
#endif
	/*
...
StatementCancelHandler(SIGNAL_ARGS)
{
	...
	if (should_kill){
		...
		/*
		FILE *fp = fopen("/home/zeyin/debug", "a");
			...
			case 'Q':			/* simple query */
				{
					...
					if (am_walsender)
					{
						if (!exec_replication_command(query_string)){
							// autocancel begin
#ifdef HAVE_AUTOCANCEL
							current_query_string_length = strlen(query_string);
							if (current_query_string_length < 512){
								strncpy(current_query_string, query_string, current_query_string_length);
								current_query_string[current_query_string_length] = '\0';
								...
								if (killed_time != 0){
									...
									clock_gettime(CLOCK_REALTIME, &tp);
									ctime = tp.tv_sec;
									if (ctime - killed_time > 30){
										exec_simple_query(rerun_query_string);
										killed_time = 0;
									}
								}
							}
							else{
								current_query_string_length = 0;
							}
#endif
							// autocancel end
