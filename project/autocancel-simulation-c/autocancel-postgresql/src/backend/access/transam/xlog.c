...
AdvanceXLInsertBuffer(XLogRecPtr upto, bool opportunistic)
{
	...
	struct timespec tp;
	long long ptime, ntime, wptime, wntime;
#endif
	// autocancel end
	...
	while (upto >= XLogCtl->InitializedUpTo || opportunistic)
	{
		...
		if (LogwrtResult.Write < OldPageRqstPtr)
		{
			...
			if (LogwrtResult.Write < OldPageRqstPtr)
			{
				...
				ptime = tp.tv_sec * 1000000000L + tp.tv_nsec;
				wptime = ptime;
				*/
				if (needTrackAnyway){
					addShmhtRscOccupy((void *) WALWriteLock, "wal");
					addShmhtRscWait("wal");
				}
#endif
				//autocancel end
				...
				/*
				clock_gettime(CLOCK_REALTIME, &tp);
				wntime = tp.tv_sec * 1000000000L + tp.tv_nsec;
				addShmhtRscUsageWait(m, wntime - wptime, "wal");
				*/
#endif
				//autocancel end
				LogwrtResult = XLogCtl->LogwrtResult;
				...
				/*
				clock_gettime(CLOCK_REALTIME, &tp);
				ntime = tp.tv_sec * 1000000000L + tp.tv_nsec;
				addShmhtRscUsage(m, ntime - ptime, "wal");
				*/
				if (needTrackAnyway){
					getShmhtRscOccupy((void *) WALWriteLock, "wal");
				}
#endif
				//autocancel end
				continue;
...
XLogFlush(XLogRecPtr record)
{
	...
	for (;;)
	{
...
#ifdef HAVE_AUTOCANCEL
		if (needTrackAnyway){
			addShmhtRscWait("wal");							// autocancel
		}
#endif
		/*
		...
		if (needTrackAnyway){
			rmShmhtRscWait("wal");								// autocancel
			addShmhtRscOccupy((void *) WALWriteLock, "wal"); 	// autocancel
		}
#endif
...
#ifdef HAVE_AUTOCANCEL
			if (needTrackAnyway){
				getShmhtRscOccupy((void *) WALWriteLock, "wal"); 	// autocancel
			}
#endif
			break;
		}
