...
#include "pthread.h" //autocancel
pthread_t monitorSelfTid, monitorPerfTid; //autocancel
...
void activate_cancel(unsigned long task_id){
	...
	fclose(fp);
	*/
	autocancel_pg_signal_backend(task_id, SIGINT);
}
#endif
// autocancel end
/*
...
PostmasterMain(int argc, char *argv[])
{
	...
	// autocancel begin
#ifdef HAVE_AUTOCANCEL
	m = createCancellableTaskShmht(128, 10);	
...
#ifdef HAVE_AUTOCANCEL
	void *res;
	closeMonitor(); 				// autocancel
	pthread_join(monitorSelfTid, &res);
	pthread_join(monitorPerfTid, &res);
	//detachShmhtCancellable();		// autocancel
#endif
	// autocancel end
...
BackendStartup(Port *port)
{
	...
	if (pid == 0)				/* child */
	{
		free(bn);
#ifdef HAVE_AUTOCANCEL
		m = createCancellableTaskShmht(20, 10);		// autocancel
		int pid = (int) getpid();					// autocancel
		addShmhtTask(m, pid, 0);						// autocancel
#endif
